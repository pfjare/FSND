...
class QuestionView extends Component {
  ...
  getByCategory = (id) => {
    $.ajax({
      ...
      error: (error) => {
        alert("Unable to load questions. Please try your request again2");
        return;
      },
    });
  };
  ...
  submitSearch = (searchTerm) => {
    $.ajax({
      url: `/questions/search?page=${this.state.page}`,
      type: "POST",
      dataType: "json",
      ...
      success: (result) => {
        this.setState({
          questions: result.questions,
          totalQuestions: result.total_questions,
          currentCategory: 0, // search category is 0
          page: 1, //reset pages
          ...
          <ul>
            {Object.keys(this.state.categories).map((id) => (
              <li
                ...
                onClick={() => {
                  this.setState(
                    { currentCategory: id, page: 1, searchTerm: null },
